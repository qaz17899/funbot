...
class StarterSelect(ui.Select["StarterSelectLayout"]):
    ...
    def __init__(self, starters: list[PokemonData], player_id: int) -> None:
    ...
    async def callback(self, interaction: Interaction) -> None:
        ...
        # Create wallet and ball inventory for player
        await PlayerWallet.get_or_create(user_id=self.player_id)
        await PlayerBallInventory.get_or_create(user_id=self.player_id)
...
class StarterSelectActionRow(ui.ActionRow["StarterSelectLayout"]):
    ...
    def __init__(self, starters: list[PokemonData], player_id: int) -> None:
        super().__init__()
        self.add_item(StarterSelect(starters, player_id))
...
class StarterSelectLayout(ui.LayoutView):
    ...
    def __init__(self, starters: list[PokemonData], player_id: int) -> None:
        ...
        container.add_item(
            ui.TextDisplay("歡迎來到寶可夢世界！請選擇以下其中一隻作為你的夥伴：")
        )
        ...
        for starter in sorted_starters:
            ...
            if starter.sprite_url:
                section = ui.Section(
                    ...
                    ui.TextDisplay(
                        f"HP: {starter.base_hp} | ATK: {starter.base_attack}"
                    ),
        ...
        container.add_item(StarterSelectActionRow(starters, player_id))
